...
function openwin(durl,dwinid,dtitle,dwidth,dheight){
        window.top.art.dialog.open(durl, {id:dwinid,lock:false,opacity:0,title:dtitle,width:dwidth,height:dheight});
}

// 副栏目
function setcatid2(obj){
	$("#setcats").find("[value='"+$("#catid").val()+"']").attr("disabled","disabled");
	var catid = $("#catid").val();
	var content = document.getElementById("setcats");
	var catidselect = $("#setcats input");
	var selvalue = '';
	var winid = 'win_'+obj;
	window.top.art.dialog({
		id:winid,
		...
		//lock: false,
		ok: function(){
			for(var i = 0 ; i < catidselect.length;i++)
			{
				if(catidselect[i].checked && catidselect[i].value!= catid){
					selvalue += (selvalue=='' ? catidselect[i].value : ','+catidselect[i].value);
				}
			}
			$("#"+obj).val(selvalue);
		}
